...
each route a small set of representative example phrases — its *profile* utterances. At request time the
selector embeds the user's message, measures how close it is to each route's utterances, and routes to
the closest-matching route. It needs only an `IEmbeddingGenerator<string, Embedding<float>>` — no extra
...
The constructor copies `routeProfiles` into a case-insensitive `Dictionary<string, string[]>`
(`_routeProfiles`), validating that every route has at least one non-blank utterance. It also unpacks
`SemanticRouterOptions` into plain fields — `_topK`, `_aggregation`, `_scoreThreshold`,
`_scoreThresholdByRoute` — and stores the optional `_defaultRoute`. Nothing is embedded yet.
...
`ctx.Routes`) so utterances belonging to a model that is no longer registered are skipped. For each
...
`SelectWinner` sorts the aggregated routes by descending score (ties by registration order) and returns
the **first route whose score meets its threshold**. The threshold is the per-route override from
`_scoreThresholdByRoute` if present, otherwise the global `_scoreThreshold` (default `0.3`):

```csharp
float threshold = _scoreThreshold;
if (_scoreThresholdByRoute?.TryGetValue(name, out float perRoute) == true)
    threshold = perRoute;
...
the `_defaultRoute` if it is still registered, otherwise `ctx.Routes[0]` (the first registered route).

### 8. Build the ordered plan — `OrderPlan`

Finally `OrderPlan` returns a `ChatRoute[]` with the **primary first**, then the remaining models
...
// Describe each route with a few representative utterances. The key is the route name
// (the same name passed to ChatRoute).
...
[
    new ChatRoute("openai:gpt-4o-mini", modelId: "gpt-4o-mini", client: gpt4oMiniClient),
    new ChatRoute("openai:gpt-5.3", modelId: "gpt-5.3", client: gpt53Client),
],
    selector: selector);
```
...
- **`ScoreThreshold` / `ScoreThresholdByRoute`.** Cosine similarity ranges from -1 to 1. A route is
  only chosen when its aggregated score meets its threshold; otherwise the `defaultRoute` (or first
  registered route) is used. Use a per-route override to hold a specific route to a higher bar. Set a
  threshold of 0 or below to always route to the best-scoring route.
...
|---|---|---|---|
| Default (pin / first) | `ChatOptions.ModelId` or `Routes[0]` | ~0 ms | trivial |
