...
public interface IChatRouteSelector
{
    ...
    /// <returns>A plan whose first route is the primary route and whose remaining routes are fallbacks.</returns>
    /// <remarks>
    /// Every route in the returned plan must be one of the exact <see cref="ChatRouteContext.Routes"/>
    /// instances passed to this call: the router matches routes by reference identity, not by value, so
    /// returning a reconstructed or otherwise unknown <c>ChatRoute</c> (even one with identical
